...
module ALU (
    ...
    always_comb begin //begins the sensitivty for comb logic and helps prevents errors
        ...
        case (alu_op) //based on what alu_op is the specific operator is chosen
            ...
            3'b101: out = src ^ src2; //xor (combines active bits exclusive to the sources)

            default: out = '0; //the value whenever no ALU operator is chosen
        endcase
        assign zero = ~(|out); //sets the zero flag by using a reducing or to see if ANY bits from out is nonzero (if any are assign zero wont flag)
    end
endmodule : ALU